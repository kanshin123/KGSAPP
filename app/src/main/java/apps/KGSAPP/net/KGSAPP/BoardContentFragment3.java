 ...
 */
public class BoardContentFragment3 extends Fragment {
    ...
    public void getListData(String sListName, final String sLastNo) {
        ...
        aq.ajax("http://kgs.yunstone.com/ajax/board.php?lat=" + lat + "&lon=" + lon + "&start=0&uuid=" + uuid + "&set=3", JSONObject.class, new AjaxCallback<JSONObject>() {
            @Override
        ...
        public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) { //필수 인터패이스
            ...
            try {
                if (holder instanceof ListItemViewHolder) {
                    ...
                    buid = mItems.getJSONObject(position).getString("buid");
                    ...
                    item_holder.mcancel.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            ...
                            alertdialog.setPositiveButton(getString(R.string.toast4), new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    //aq.ajax(AppVar.API_ROOT + sListName + ".php?key=" + AppVar.API_KEY + "&last=" + sLastNo, JSONObject.class, new AjaxCallback<JSONObject>() {
                                    aq.ajax("http://kgs.yunstone.com/ajax/boarddel.php?mid=" + buid + "&uuid=" + uuid , JSONObject.class, new AjaxCallback<JSONObject>() {
                                        @Override
                                        public void callback(String url, JSONObject object, AjaxStatus status) {
                                            if (object != null) {
                                                //로딩표시 지움.
                                                ...
                                                try {
                                                    ...
                                                    for (int i = 0; i < arrData.length(); i++) {
                                                        ...
                                                        if (logincheck.equals("0")) {
                                                            Toast.makeText(getContext(), getString(R.string.board11), Toast.LENGTH_LONG).show();
                                                        } else {
